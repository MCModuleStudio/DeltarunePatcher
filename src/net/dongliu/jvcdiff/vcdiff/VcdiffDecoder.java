...
public class VcdiffDecoder {
    ...
    private boolean useSecondaryCompress;

	private XZInputStream addRunDataStream, instructionsStream, addressesStream;
    ...
    private void readHeader() throws IOException, PatchException {
        ...
        int headerIndicator = patchStream.read();
        if ((headerIndicator & 1) != 0) {
            // secondary compress.
            this.useSecondaryCompress = true;
            int compressorId = patchStream.read();
            if (compressorId != 0x02 /* LZMA */) {
    ...
    private boolean decodeWindow() throws IOException, PatchException {
        ...
        if (deltaIndicator != 0 && !this.useSecondaryCompress) {
        ...
        byte[] addRunData;
        if ((deltaIndicator & 1) == 0) {
            addRunData = IOUtils.readBytes(patchStream, addRunDataLen);
        } else {
            int decompressedSize = IOUtils.read7bitIntBE(patchStream);
            XZInputStream addRunDataStream = this.addRunDataStream;
            if (addRunDataStream == null) {
                addRunDataStream = this.addRunDataStream = new XZInputStream(patchStream);
            }
            addRunData = IOUtils.readBytes(addRunDataStream, decompressedSize);
//            addRunData = IOUtils.readBytes(new XZInputStream(new LimitedInputStream(patchStream, addRunDataLen)), decompressedSize);
        }
        int addRunDataIndex = 0;
        // Instructions and sizes section
        byte[] instructions;
        if ((deltaIndicator & 2) == 0) {
            instructions = IOUtils.readBytes(patchStream, instructionsLen);
        } else {
            int decompressedSize = IOUtils.read7bitIntBE(patchStream);
            XZInputStream instructionsStream = this.instructionsStream;
            if (instructionsStream == null) {
                instructionsStream = this.instructionsStream = new XZInputStream(patchStream);
            }
            instructions = IOUtils.readBytes(instructionsStream, decompressedSize);
//            instructions = IOUtils.readBytes(new XZInputStream(new LimitedInputStream(patchStream, instructionsLen)), decompressedSize);
        }
        // Addresses section for COPYs
        byte[] addresses;
        if ((deltaIndicator & 4) == 0) {
            addresses = IOUtils.readBytes(patchStream, addressesLen);
        } else {
            int decompressedSize = IOUtils.read7bitIntBE(patchStream);
            XZInputStream addressesStream = this.addressesStream;
            if (addressesStream == null) {
                addressesStream = this.addressesStream = new XZInputStream(patchStream);
            }
            addresses = IOUtils.readBytes(addressesStream, decompressedSize);
//            addresses = IOUtils.readBytes(new XZInputStream(new LimitedInputStream(patchStream, addressesLen)), decompressedSize);
        }
    ...
    private void check(int checksumInFile, byte[] targetData) throws PatchException {
        Adler32 adler32 = new Adler32();
        adler32.update(targetData);
        if ((checksumInFile & 0xFFFFFFFFL) != adler32.getValue())
            throw new PatchException(String.format("Incorrect checksum, expected %08x, got %08x.", (checksumInFile & 0xFFFFFFFFL), adler32.getValue()));
    }
}
